...
function SignUp() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (success) {
        addAlert({ type: "success", message: "Signed Up Successfully. Please sign in now.", timeout: 5 });
    ...
    <div className='bg-slate-200 h-full w-full absolute flex justify-center items-center'>
      <div className='w-4/6 h-4/6 flex justify-center items-center'>
        <div className='inherit h-full w-1/2 bg-white border rounded-l-md flex justify-center items-center'>
          <form className='relative h-3/4 w-5/6 block' onSubmit={handleSubmit}>
            ...
            <div className='block mt-10 h-1/6'>
              ...
              <input className='mt-2 border border-gray-900 rounded-sm w-full h-1/2 p-2' type='text' name='email' onChange={handleChange} required></input>
            </div>
            <div className='block mt-5 h-1/6'>
              <span className='text-lg text-black'>Password</span>
              <input className='mt-2 border border-gray-900 rounded-sm w-full h-1/2 p-2' type='password' name='password' onChange={handleChange} required></input>
            </div>
            ...
            <button type='submit' className='w-full h-10 mt-10 bg-red-700 flex justify-content items-center'>
        ...
        <div className='inherit h-full w-1/2 border rounded-r-md flex justify-center items-center relative'>
          <img className='inherit h-full w-full object-fill border rounded-r-md z-0 absolute' src={require('../assets/login-wave-background.png')} alt='login background'></img>
